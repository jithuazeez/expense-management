...
async def get_expenses(category: Optional[str] = None, db: Session = Depends(get_db)):
    if category:
...
async def get_total_salary(db: Session = Depends(get_db)):
